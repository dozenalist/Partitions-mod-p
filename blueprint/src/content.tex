...
\begin{lemma}
  ...
  \uses{def:f_l, def:mulpow, lem:non_diag_vanish} 
...
\begin{definition} 
  ...
  \lean{eventuallyEq}
  \leanok
  Let $\alpha$ be a field, and let $f, g : \mathbb{N} \to \alpha \llbracket X \rrbracket$ be two sequences of power series with coefficients in $\alpha$.
  ...
  the jth coefficient of $f(k)$ is equal to the jth coefficient of $g(k)$. This is an equivalence relation.
\end{definition}
...
\begin{lemma}
  ...
  \leanok
  Let $m, N \in \mathbb{N}$. Let $f : \mathbb{N} \to \alpha \llbracket X \rrbracket$ be a sequence 
...
\begin{theorem}
  ...
  \uses{def:f_l, thm:DeltaProduct_eventually_sum, thm:eventuallyEq_mul}
...
\begin{theorem} 
  ...
  \uses{def:U_Operator, def:ramanujan_congruence, thm:fl_Product_eventually_sum, 
    thm:partitionProduct_eq, lem:coeff_zero_of_ndvd, lem:coeff_mul_shift_of_zero, lem:coeff_sum_squash}
  \lean{flu_eq_zero}
  \leanok
