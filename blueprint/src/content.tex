...
\begin{theorem}
  \label{thm:num_two_mul_bernoulli}
  \lean{IntegerModularForm.num_two_mul_bernoulli}

  $\frac{2k}{bernoulli_{2k}}$ is an integer
...
\begin{definition} [Eisenstein Series]
  \label{def:Eis}
  \uses{def:IntegerModularForm, theorem:num_two_mul_bernoulli}
  \lean{IntegerModularForm.Eis}
  \leanok
  For $k \ge 2 \in \mathbb{N}$, the Eisenstein series $E_k$ is an integer modular form of weight $2k$ defined by 
  \[ E_k = 1 - \frac{2k}{B_{2k}} \sum_{n = 1}^{\infty} \sigma_{k - 1} (n) q ^ n \]
  where $B_k$ is the kth Bernoulli number and $\sigma_{k} (n)$ is the sum of the kth powers of the divisors of $n$.
\end{definition}


\begin{theorem}
  \label{thm:dvd_coeff_succ_El}
  \uses{def:Eis}
  \lean{IntegerModularForm.dvd_coeff_succ_El}

  For $n > 0$, $\ell$ divides the $n$th coefficient in the q-expansion of $Eis_{\ell - 1}$.
\end{theorem}


\begin{definition} [Modular Form Mod $\ell$]
  \label{def:ModularFormMod}
  \uses{def:IntegerModularForm, thm:dvd_coeff_succ_El}
  \lean{ModularFormMod}
  \leanok
...
\begin{definition} [Filtration]
  ...
  \lean{ModularFormMod.Filtration}
  \leanok
  Let $a$ be a modular form mod $\ell$. The filtration of $a, \omega (a), $ 
...
\begin{definition} [multiplication and exponentiation]
  \label{def:mulpow}
  \uses{def:ModularFormMod}
  \lean{ModularFormMod.mul, ModularFormMod.pow}
  \leanok
  It's worth stating how multiplication and exponentiation are defined here, because they are not defined in the normal way.
...
\begin{theorem}[Pow Prime] 
  \label{thm:coeff_pow_Prime}
  \uses{def:mulpow}
  \lean{ModularFormMod.coeff_pow_Prime}
  \leanok
...
\begin{theorem}
  \label{thm:UOp_pow_l}
  \uses{thm:coeff_pow_Prime, def:UOp, def:Theta}
  \lean{UOp_pow_l}
  \leanok
...
\begin{theorem}
  \label{thm:Reduce_of_reduce}
  \uses{def:ModularFormMod, def:IntegerModularForm}
  \lean{ModularFormMod.Reduce_of_reduce}
  \leanok

  If an integer modular form of $a$ weight $k$ reduces to a modular form mod $\ell$ of weight $j$ 
  then we must have $k \equiv j \pmod (\ell - 1)$

\end{theorem}

\begin{theorem}
  \label{thm:Filtration_congruence}
  \uses{def:Filtration, thm:Reduce_of_reduce}
  \lean{ModularFormMod.Filtration_congruence}
  \leanok
...
\begin{definition}[Delta]
  ...
  $\Delta$, is defined to be the the reduction of $\Delta$.
\end{definition}

\begin{definition}
  \label{def:fl}
  \uses{def:Delta}
  \lean{IntegerModularForm.fl}
  \leanok
...
\begin{definition}
  ...
  \lean{dim}
  \leanok
  For $k \in \mathbb{N}$, define dim$(k) = \lfloor k / 6 \rfloor +         
        \begin{cases}
            0 & \text{ if } k\equiv 1\pmod{6} \\
            1 & \text{ else }
        \end{cases} $. \\
...
\begin{definition}
  \label{def:G}
  \uses{def:Eis, def:Delta, def:dim}
  \lean{IntegerModularForm.G}

  For $ k \ne 1$ and $c < $ dim$(k)$, 
  write $k$ uniquely as $k = 6\ell + k'$, where $k' \in \{0, 2, 3, 4, 5, 7\}$.
  $G_{k,\, c}$ is a modular form of weight $2k$ defined as 
...
\begin{definition} [ord]
  \label{def:ord}
  \uses{def:IntegerModularForm}
  \lean{IntegerModularForm.ord}
  \leanok
  Let $b$ be an integer modular form. The order of $b$, ord$(b)$, is defined as the minimum $n \in \mathbb{N}$ such that $b(n) \neq 0$.
  This is the order of vanishing of $b$ at infinity.
\end{definition}

\begin{theorem}
  \label{thm:ord_mul}
  \uses{def:ord, def:mulpow}
  \lean{IntegerModularForm.ord_mul}
  \leanok
  ord$(a \cdot b) = $ ord$(a) + $ ord$(b)$.
\end{theorem}
\begin{proof}
  \leanok
\end{proof}
...
\begin{theorem}
  \label{thm:coeff_mul_ord}
  \uses{thm:ord_mul}
  \lean{IntegerModularForm.coeff_mul_ord}
  \leanok
  $(a \cdot b) ($ord$(a) + $ ord$(b)) = a($ ord$(a)) \cdot b($ ord$(b))$.
\end{theorem}
\begin{proof}
  \leanok
\end{proof}
...
% \begin{theorem}
%   \label{thm:ord_pow_ord}
%   \uses{def:ord, thm:ord_mul}
%   \lean{IntegerModularForm.ord_pow_ord}
%   \leanok
%   $a ^ m (m \cdot $ ord$(a)) = a($ ord$(a)) ^ m$.
% \end{theorem}
% \begin{proof}
%   \leanok
% \end{proof}
...
\begin{theorem}
  \label{thm:ord_Delta}
  \uses{def:Delta, def:ord}
  \lean{IntegerModularForm.ord_Delta}
  \leanok
  ord$(\Delta) = 1$.
\end{theorem}
\begin{proof}
...
\begin{theorem}
  \label{thm:ord_Eis}
  \uses{def:Eis, def:ord}
  \lean{IntegerModularForm.ord_Eis}
  \leanok
  For any k, ord$(E_k) = 0$.
\end{theorem}
\begin{proof}
...
\begin{theorem}
  \label{thm:ord_G}
  \uses{def:G, thm:ord_Delta, thm:ord_Eis, thm:ord_mul}
  \lean{IntegerModularForm.ord_G}
  \leanok
  ord$(G_{k, \, c}) = c$.
\end{theorem}
\begin{proof}
...
\begin{theorem}
  \label{thm:G_ord_G}
  \uses{thm:ord_G, thm:coeff_mul_ord, thm:coeff_mul_ord}
  \lean{IntegerModularForm.G_ord_G}
  \leanok
  $G_{k,\, c} (c) = 1$.
\end{theorem}
\begin{proof}
...
\begin{theorem}
  \label{thm:coeff_trunc_injective}
  \uses{def:G, thm:ord_G, def:dim}
  \lean{IntegerModularForm.coeff_trunc_injective}
  \leanok
  If the coefficients of $a$ and $b$ match up to dim$(k)$, then $a = b$.
\end{theorem}
\begin{proof}
  \leanok
\end{proof}
...
\begin{theorem}
  \label{thm:eq_G_of_ord_max}
  \uses{thm:coeff_trunc_injective, thm:G_ord_G}
  \lean{IntegerModularForm.eq_G_of_ord_max}
  \leanok
  Let $a$ be a modular form of weight $2k$. If ord$(a) = $ dim$(k) - 1$ then
  $a = a ($dim$(k) - 1) G_{k, \, \text{dim} (k) - 1}.$ 
\end{theorem}
\begin{proof}
  \leanok
\end{proof}
...
\begin{theorem}
  \label{thm:exists_maximal_Reduce}
  \uses{thm:G_ord_G, thm:Reduce_of_reduce, thm:coeff_trunc_injective}
  \lean{ModularFormMod.exists_maximal_Reduce}
  \leanok
  Let $a$ be a modular form mod $\ell$ of weight $k$. If ord$(a) \ge m$, then there exists an
  integer modular form $b$ of weight $k$ such that \\
  (1) ord$(b) \ge m$, and \\
...
\begin{lemma}
  \label{lem:Filt_fl}
  \uses{def:fl, thm:exists_maximal_Reduce}
  \lean{Filt_fl}
  \leanok
...
\begin{theorem}
  ...
  \uses{def:fl, thm:Filtration_congruence, thm:exists_maximal_Reduce}
...
\begin{theorem}
  \label{thm:Filtration_Theta_pow_sub_one}
  \uses{thm:UOp_pow_l, def:Filtration}
  \lean{Filtration_Theta_pow_sub_one}
  \leanok
  The conclusion of Lemma 3.3 \\
  If $f_\ell | U = 0$ then $\omega (\Theta^{\ell - 1} f_\ell) = \frac{\ell ^ 2 - 1}{2}$
\end{theorem}
\begin{proof}
  \leanok
\end{proof}
...
\begin{lemma}
  \label{lem:Filt_Theta_congruence_of_dvd} 
  \uses{thm:Reduce_of_reduce, thm:Filt_Theta_bound, thm:Filt_Theta_iff}
  \lean{Filt_Theta_congruence_of_dvd}
  \leanok
...
\begin{theorem}
  ...
  \uses{thm:Filtration_Theta_pow_sub_one, lem:Filt_fl, thm:le_Filt_Theta_fl}
...
\begin{definition}
  \label{def:alpha}
  \uses{lem:Filt_Theta_lel_add_one_div_two, lem:Filt_Theta_congruence_of_dvd, thm:Filt_Theta_l_add_one_div_two}
  \lean{alpha}
  \leanok
...
\begin{lemma}
  ...
  \uses{def:fl, def:mulpow} 
  \lean{ModularFormMod.fl_delta_add_one}
  \leanok
...
\begin{definition} 
  ...
  \lean{PowerSeries.eventuallyEq}
  \leanok
  Let $\alpha$ be a commutative semiring, and let $f, g : \mathbb{N} \to \alpha [[X]]$ be two sequences of power series with coefficients in $\alpha$.
...
\begin{theorem}
  ...
  \uses{def:eventuallyEq}
  \lean{PowerSeries.eventuallyEq_mul}
  \leanok
...
\begin{lemma}
  ...
  \lean{PowerSeries.coeff_sum_X_pow_mul}
  \leanok
  Let $n, N, \ell \in \mathbb{N}$ with $n \: < \: N$ and $\ell \: > \: 0$. Let $a : \mathbb{N} \to \alpha$ be a sequence with coefficients in a semiring $\alpha$.
...
\begin{lemma}
  ...
  \uses{lem:coeff_sum_X_pow_mul}
  \lean{PowerSeries.coeff_sum_squash}
  \leanok
...
\begin{lemma}
  ...
  \lean{PowerSeries.coeff_mul_shift_of_zero}
  \leanok
...
\begin{lemma}
  ...
  \lean{PowerSeries.coeff_zero_of_ndvd}
  \leanok
...
\begin{lemma}
  ...
  \uses{lem:coeff_zero_of_ndvd}
  \lean{PowerSeries.prod_eq_sum}
  \leanok
...
\begin{theorem}
  ...
  \uses{def:Delta, def:eventuallyEq}
  \lean{IntegerModularForm.DeltaProduct_eventually_sum}
  \leanok
...
\begin{theorem}
  \label{thm:flProduct_eventually_sum}
  \uses{def:fl, thm:DeltaProduct_eventually_sum, thm:eventuallyEq_mul}
  \lean{IntegerModularForm.flProduct_eventually_sum}
  \leanok
  ...
  In other words,
  \[ (X ^ {\delta_\ell} \prod_{i \, \le \: \cdot} (1 - X ^ {i + 1}) ^ {24 \delta_\ell} \longrightarrow 
...
\begin{theorem} 
  ...
  \uses{def:UOp, def:ramanujan_congruence, thm:flProduct_eventually_sum, 
    thm:partitionProduct_eq, lem:prod_eq_sum, lem:coeff_mul_shift_of_zero, lem:coeff_sum_squash}
  \lean{flu_eq_zero}
  \leanok
...
\begin{theorem}
  \label{thm:Eis_four_one}
  \uses{def:Eis}
  \lean{IntegerModularForm.Eis_four_one}
  \leanok
  $E_4 (1) = 240$.
\end{theorem}
\begin{proof}
...
\begin{theorem}
  \label{thm:G_delta_add_one}
  \uses{def:G, thm:coeff_mul_ord, thm:ord_Delta, lem:fl_delta_add_one, thm:Eis_four_one}
  \lean{G_delta_add_one}
  \leanok
  $G_{6\delta_\ell + 2, \, \delta_\ell} (\delta_\ell + 1) \equiv 241 \pmod \ell$.
\end{theorem}
\begin{proof}
  \leanok
\end{proof}
...
\begin{theorem}
  \label{thm:Theta_l_add_three_div_two}
  \uses{def:fl, thm:exists_maximal_Reduce, thm:eq_G_of_ord_max, 
    thm:Reduce_of_reduce, thm:Filt_Theta_l_add_three_div_two}
  \lean{Theta_l_add_three_div_two}
  \leanok
...
\begin{theorem}
  ...
  \uses{thm:Theta_l_add_three_div_two, thm:G_delta_add_one} 
  \lean{Theta_l_add_three_div_two_eq_241}
  \leanok
...
\begin{theorem}
  ...
  \uses{thm:Theta_l_add_three_div_two_eq_241, lem:fl_delta_add_one}
  \lean{flu_ne_zero}
  \leanok
...
\begin{theorem}
  ...
  \uses{thm:flu_eq_zero, thm:flu_ne_zero}
  \lean{MainResult}
  \leanok
  If $\ell \ge 13$ is prime, then there does not exist a ramanujan congruence mod $\ell$.
\end{theorem}
\begin{proof}
  \leanok
\end{proof}
