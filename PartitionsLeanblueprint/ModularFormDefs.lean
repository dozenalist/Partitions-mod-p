...
--   summable : Summable (fun n ↦ fourier.coeff n * q ^ n)
--   modular : ModularFormClass k (∑' n, fourier.coeff n * q ^ n)
...
--   coe a := fun n => a.fourier.coeff n
--   coe_injective' a b c := by cases a; cases b; congr; ext n; exact congrFun c n
...
--     modular := by
--       simp
--       have : ∑' n, ((a.fourier.coeff _ n) + (b.fourier.coeff _ n)) * q ^ n = ∑' n, (a.fourier.coeff _ n) * q ^ n + ∑' n,  (b.fourier.coeff _ n) * q ^ n := by
...
-- @[simp]
-- theorem coe_apply (f : IntegerModularForm k) (n : ℕ) : f.fourier.coeff  n = f n := rfl
...
--   fun n ↦ (f.fourier * g.fourier).coeff _ n := rfl
...
--   (f.fourier * g.fourier).coeff _ n := rfl
...
-- theorem coe_Ipow (a : IntegerModularForm k) (j : ℕ) : ⇑(Ipow a j) = fun n ↦ (a.fourier ^ j).coeff _ n := rfl

-- theorem Ipow_apply (a : IntegerModularForm k) (j n : ℕ) : (Ipow a j) n = (a.fourier ^ j).coeff _ n := rfl


-- #check PowerSeries.C

-- @[simp]
-- theorem Iconst_zero (x : ℤ) : (Iconst x) 0 = x := by simp [Iconst, ← coe_apply]

-- @[simp]
-- theorem Iconst_succ (x : ℤ) (n : ℕ) : (Iconst x) n.succ = 0 := by simp [Iconst, ← coe_apply ← PowerSeries.C];
