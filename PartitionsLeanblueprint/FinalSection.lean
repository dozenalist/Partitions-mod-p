...
/- This file proves from Lemma 3.3 (line 3.5) to line 3.8
...
lemma l_death_ineq : (ℓ + 5) / 2 * (ℓ - 1) ≤ (ℓ + 3) / 2 * (ℓ + 1) := calc
...
lemma alpha_equal [Fact (f ℓ |𝓤 = 0)] : α + 1 = (ℓ + 5) / 2 := by
  ...
  contrapose! lel; calc
    ...
    _ ≤ (ℓ + 5) / 2 + ((ℓ - 7)/2 + 1) := by
      apply add_le_add_left; apply add_le_add_right j_bound
    _ = (ℓ + 5) / 2 + (ℓ - 5)/2 := by
