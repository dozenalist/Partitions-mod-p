...
-- def Reduce_Set (PowerSeries (ZMod ℓ)) = {x : ⨁ i, IntegerModularForm i |  }


-- structure ModularFormMod ℓ (k : ZMod (ℓ - 1))

-- fourier : PowerSeries (ZMod ℓ)

-- nempty : (Reduce_Set fourier).nonempty
...
-- ModularFormMod ℓ ↑(j:ℤ)

-- ModularFormMod ℓ (j : ZMod (ℓ - 1))
